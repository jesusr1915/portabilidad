...
export class ViewConsultaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.subscription = this._menuMan.getMessage().subscribe((message) => {
  ...
  candidatoOtp() {
    ...
    this.loginServices.postAccesoOtp(json).subscribe(
      (res) => {
        // console.log(res);
  ...
  private loadInfo() {
    ...
    this.loginServices.getSaldos().subscribe(
      (res) => {
        ...
        this.loginServices.postDetalleConsulta(datos).subscribe(
          (res) => {
            this.consultaPN = res.dto;
            let myAccounts = this.matchAccounts(
              this.saldosCuentas,
              this.consultaPN
            );
            ...
            this.loginServices.getConsultaRFC().subscribe(
              (res2) => {
                ...
                localStorage.setItem(
                  "birthday",
                  this.formatDate(res2.dto.fechaNacimiento)
                );
              ...
              (err2) => {
                this.mainService.showAlert({
                  title: "Portabilidad de nómina",
                  body:
                    "Por el momento el servicio no está disponible, por favor intenta de nuevo más tarde",
                  buttonAccept: "Aceptar",
                  item: { exit: 1 },
                });
              }
  ...
  private filterMoves(idBtn: number) {
    ...
    for (let moves of this.allMov) {
      ...
      let maskCuenta =
        moves.cuentaCliente.substr(0, 2) +
        "**" +
        moves.cuentaCliente.substr(accountLenght - 4, accountLenght);
      let newMove = {
      ...
      if (newMove.tipoSolicitud === tipoSolicitud) {
        if (localStorage.getItem("numeroCuenta") === newMove.numeroCuenta) {
          ...
          localStorage.getItem("cardDisponible") === "Todas las cuentas"
        ) {
          newMoves.push(newMove);
        }
