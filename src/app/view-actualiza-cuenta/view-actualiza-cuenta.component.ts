...
@Component({
  ...
  styleUrls: [
    "./view-actualiza-cuenta.component.scss",
    "../app.component.scss",
  ],
})
export class ViewActualizaCuentaComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.errorService(
      "",
      "Usted está iniciando el proceso de cambio de cuenta inscrita a Santander Plus. Esta opción le permitirá recibir los beneficios de Santander Plus en una cuenta diferente a la seleccionada en el proceso de inscripción. <br/><br/> Por favor cualquier duda o aclaración comuníquese a la línea de Santander Plus al 01800 0101123.",
      "Aceptar",
      "",
      3
    );
  ...
  private loadInfo() {
    this.loginServices.postModificaSP().subscribe(
      (res) => {
          ...
          this.loginServices.getSaldos().subscribe(
            (res) => {
              for (let cta of res.dto.saldoPesos) {
                if (
                  cta.numeroCuenta === localStorage.getItem("ctaSantanderPlus")
                ) {
                  if (cta.alias === "") {
                  ...
                  this.valueNumCuenta =
                    cta.numeroCuenta.substr(0, 2) +
                    "**" +
                    cta.numeroCuenta.substr(accountLenght - 4, accountLenght);
                  this.valueCuentaMovil = cta.cuentaMovil;
                  localStorage.setItem("cardAliasSP", this.valueAlias);
                  localStorage.setItem(
                    "cardDisponibleSP",
                    this.valueDisponible
                  );
                  ...
                  localStorage.setItem(
                    "cardCuentaMovilSP",
                    this.valueCuentaMovil
                  );

                  this.cuentaEnCeros =
                    parseFloat(this.valueDisponible.replace(",", "")) <= 0
                      ? true
                      : false;
                }
              }
  ...
  private loadMock() {
    this.loginServices.getModificaSPMock().subscribe(
      (res) => {
        if (res.dto.isSantanderPlus) {
          // this.ctaSantanderPlus = res.dto.cuenta;
          localStorage.setItem("isSantanderPlus", "true");
    ...
    this.loginServices.getSaldosSPMock().subscribe(
      (res) => {
        ...
        this.cuentaEnCeros =
          parseFloat(
            localStorage.getItem("cardDisponibleSP").replace(",", "")
          ) <= 0
            ? true
            : false;
